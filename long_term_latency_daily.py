...
def long_term_latency():
  ...
  query = (
              f"SELECT * FROM `smu-benchmarking.reporting.historic_latency` "
  ...
  gb = results.groupby(['sending_zone','receiving_zone'])
  ...
  dataframe_to_insert_columns = ['entry_date',
                                 ...
                                 'most_recent_moving_average','lin_reg_coef', 'alert_string']
  ...
  for group in groups:
      ...
      for timespan_days in [7,30,90,180]:
          ...
          # print(f'coef_: {linear_regressor.coef_[0][0]}')
          ...
          new_row = {'entry_date': end_date,
                     ...
                     'alert_string': f'Latency moving average changed by {change_from_start:.3f} {unit} in the past {timespan_days} days'
                    }
  ...
  insert_table_id = 'smu-benchmarking.reporting.long_term_trend'
